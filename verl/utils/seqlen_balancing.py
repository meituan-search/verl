...
def get_prefix_balanced_partitions(
    ...
    # Workload metric: same quadratic proxy as calculate_workload, applied to the
    # deduplicated flat token count instead of raw length.
    group_workloads = [int(calculate_workload(torch.tensor([eff])).item()) for _, eff in groups]

    if len(groups) <= k_partitions:
        # Fewer groups than partitions — assign each group to its own partition,
        # distribute leftover groups if any, pad with empty lists.
