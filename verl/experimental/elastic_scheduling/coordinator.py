...
Legacy components (CongestionMonitor, ResourceCoordinator, CoordinatorLoop) are
...
# ============================================================================
# New Elastic Coordinator (integrates with ElasticRollouter and ElasticTrainer)
# ============================================================================
...
class ElasticCoordinator:
    ...
    def register_elastic_resource(
        self,
        resource_id: str,
        worker_handles: list,
        initial_mode: str = "rollout",
    ):
        """Register a new elastic resource for coordination."""
        self._resources[resource_id] = ElasticResourceInfo(
            resource_id=resource_id,
            current_mode=initial_mode,
            worker_handles=worker_handles,
        )
        logger.info(f"[ElasticCoordinator] Registered elastic resource: {resource_id} ({initial_mode})")

    def update_resource_param_version(self, resource_id: str, param_version: int):
        """Update the tracked parameter version for a resource."""
        if resource_id in self._resources:
            self._resources[resource_id].param_version = param_version
    ...
    async def _create_rollout_replica_from_handles(self, resource_id: str, handles: list):
        """
        Create a RolloutReplica-compatible object from worker handles.

        In the elastic scheduling setup, each elastic resource has a set of
        Ray actor handles. We wrap them into a RolloutReplica-like object
        that the ElasticAgentLoopManager can use.
        """
        if not handles:
            logger.warning(f"[ElasticCoordinator] No handles for {resource_id}, cannot create replica")
            return None

        # Import the replica class
        try:
            # Try to get server address from workers
            # Workers should have their vLLM/SGLang server initialized
            server_address = None
            server_handle = None

            for handle in handles:
                try:
                    addr = ray.get(handle.get_rollout_server_address.remote())
                    if addr:
                        server_address = addr
            ...
            if server_address is None:
                logger.warning(
                    f"[ElasticCoordinator] Could not get server address for {resource_id}. "
                    f"Creating simple wrapper without server address."
                )

            # Create a minimal rollout replica wrapper
            # This wraps worker handles and the server handle/address
            # so that ElasticAgentLoopManager can manage it.

            class ElasticRolloutReplica:
                """
                Minimal RolloutReplica-compatible wrapper for elastic workers.

                Provides the interface expected by ElasticAgentLoopManager:
                - _server_address: address of the rollout server
                - _server_handle: Ray handle to the rollout server
                - workers: list of Ray actor handles
                - abort_all_requests(): stop in-flight requests
                - resume_generation(): allow new requests
                """

                def __init__(self, workers, server_address=None, server_handle=None):
                ...
                async def abort_all_requests(self):
                    ...
                    if self._server_handle is not None:
                        try:
                            ray.get(self._server_handle.abort_all_requests.remote())
                        except Exception as e:
                            logger.warning(f"[ElasticReplica] abort_all_requests failed: {e}")

                async def resume_generation(self):
                    """Allow new generation requests."""
                    if self._server_handle is not None:
                        try:
                            ray.get(self._server_handle.resume_generation.remote())
                        except Exception as e:
                            logger.warning(f"[ElasticReplica] resume_generation failed: {e}")

                async def init_standalone(self):
                    """No-op for elastic replicas (already initialized)."""
                    pass

            replica = ElasticRolloutReplica(
                workers=handles,
            ...
            logger.error(f"[ElasticCoordinator] Failed to create rollout replica: {e}")
            return None
    ...
    # =========================================================================
    # Pre-Sync Hook (called before parameter sync)
    # =========================================================================

    async def on_before_parameter_sync(self, param_version: int):
        """
        Hook called by ElasticRollouter just before parameter sync.

        This is the key coordination point: any pending role switches
        are executed here, ensuring:
        1. New rollout replicas are in place BEFORE they receive new params
        2. Resources leaving rollout are removed BEFORE the new params are broadcast

        Args:
            param_version: The current parameter version being synced
        """
        logger.info(f"[ElasticCoordinator] Pre-sync hook (version={param_version}). Checking for pending switches...")

        # Force a coordination check at sync time
        # (Higher priority than interval-based checks)
        async with self._switch_lock:
            queue_size, queue_capacity = await self._poll_queue_metrics()
            queue_util = queue_size / max(queue_capacity, 1)
            ...
            if action != "stable" and confidence >= self.confidence_threshold:
                logger.info(f"[ElasticCoordinator] Executing pre-sync action: {action} (confidence={confidence:.2f})")
                if action == "scale_rollout":
                    await self._switch_train_to_rollout()
                elif action == "scale_train":
                    await self._switch_rollout_to_train()

        logger.info("[ElasticCoordinator] Pre-sync hook complete")
