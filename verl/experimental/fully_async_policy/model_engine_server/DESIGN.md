# model_engine_server 设计文档

## 概述

`model_engine_server` 是一个独立的推理服务，用于在 fully-async PPO 训练中实时计算历史 log 概率（old log probabilities）。它在 rollout 生成期间与 vLLM/SGLang 并行运行，使用与 Trainer 权重同步的 Megatron/FSDP 训练引擎执行前向推理。
...
```
RolloutReplica (ABC)
└── ModelEngineReplica          # 副本生命周期管理，与 vLLM/SGLang Replica 对称

CheckpointEngineWorker (Worker)
└── ModelEngineWorker           # GPU 算子执行单元，参与 NCCL 权重同步

BaseRollout (ABC)
└── ModelEngineServerAdapter    # NCCL 权重接收 → CPU 暂存 → TrainingWorker 写入

ModelEngineServer (Ray Actor)   # 请求批量调度器，实现 abort/resume 协议
```
...
| 组件 | 文件 |
|------|------|
| `ModelEngineServerAdapter` | `verl/experimental/fully_async_policy/model_engine_server/model_engine_server.py` |
| `ModelEngineWorker` | `verl/experimental/fully_async_policy/model_engine_server/model_engine_server.py` |
| `ModelEngineServer` | `verl/experimental/fully_async_policy/model_engine_server/model_engine_server.py` |
| `ModelEngineReplica` | `verl/experimental/fully_async_policy/model_engine_server/model_engine_server.py` |
...
fully_async_main._initialize_components()
  ...
  │         └─ actor_wg.init_model()  (Trainer 侧，不涉及 model_engine_server)
  │
       ...
       └─ FullyAsyncRollouter._init_async_rollout_manager()
            └─ FullyAsyncAgentLoopManager.create()
                 └─ _initialize_llm_servers()
                      ...
                      └─ if config.model_engine_server.enable_standalone:
                           _init_model_engine_replica()
                             │
                             ├─ ModelEngineReplica(replica_rank, full_config, ModelEngineWorker)
                             │    └─ __init__: 从 full_config.model_engine_server 读取
                             ...
                             └─ replica.init_standalone()
                                  ...
                                  │
                                  ├─ RayWorkerGroup(resource_pool, ModelEngineWorker)
                                  │    └─ 每个 GPU 启动一个 ModelEngineWorker Ray Actor
                                  │
                                  └─ launch_servers()
                                       ├─ worker_group.init_model()
                                       │    └─ ModelEngineWorker.init_model() [ONE_TO_ALL]
                                       │         └─ ModelEngineServerAdapter.init_model()
                                       │              ├─ 重映射 config key
                                       │              │  (micro_batch_size → ppo_*)
                                       │              ├─ TrainingWorker(TrainingWorkerConfig)
                                       │              └─ training_worker.reset() (加载模型权重)
                                       │
                                       └─ ModelEngineServer.remote(worker_group, cfg)
                                            ├─ 启动 _batch_consumer Task
                                            └─ self.servers = [server]

                 └─ _init_agent_loop_workers()
                      └─ FullyAsyncAgentLoopWorker.remote(..., model_engine_server_handle)
                           └─ FullyAsyncLLMServerManager(model_engine_server_handle=server)
```

**关键设计**：
- `ModelEngineReplica.init_standalone()` 自建独立 Ray resource pool，与 vLLM/SGLang Replica 完全对称
- `init_model()` 延迟到 Ray Actor 启动后执行，避免在 `__init__` 时占用 GPU 内存
- `ModelEngineReplica` 追加到 `rollout_replicas`，使 Trainer 的 `CheckpointEngineManager` 通过 `rollouter.get_replicas()` 自动获取并管理其权重同步
            ...
            └─ while True:  (partial rollout 循环)
                 ...
                 └─ if not validate:
                      _recompute_old_log_prob(output, prompt_ids, temperature)
                        │
                        ├─ if model_engine_server_handle is None: return  (跳过)
                        │
                        ├─ 构造 TensorDict:
                        ...
                        │
                        └─ await server_handle.compute_log_prob.remote(data_td)
                               │
                               └─ ModelEngineServer.compute_log_prob(data_td)
                                    ├─ _drain_done.wait()  (等待未在更新权重)
                                    ├─ 入队 _request_queue
                                    └─ await future  (等待批量推理结果)

ModelEngineServer._batch_consumer (后台 Task)
            ...
            └─ async with _infer_lock:
                 infer_batch(batched_data)
                   ...
                   └─ worker_group.compute_log_prob(data)  [ND_COMPUTE]
                        └─ ModelEngineWorker.compute_log_prob(data)  [同步]
                             └─ ModelEngineServerAdapter.compute_log_prob(data)  [同步]
...
FullyAsyncTrainer._fit_update_weights()
  └─ checkpoint_manager.update_weights(global_steps)
       ...
       │           = [vLLMReplica(s), ..., ModelEngineReplica]
       │
       ├─ ① for replica in replicas:
       │       replica.abort_all_requests()
       │         └─ vLLM/SGLang: pause_generation()，保存 in-flight 请求
       │         └─ ModelEngineReplica: no-op
       │              ↑ 此窗口期：被 vLLM/SGLang partial 打断的样本
       │                仍可向 ModelEngineServer 发送请求，用当前权重计算 log prob
       │
       ├─ ② for replica in replicas:
       │       replica.sleep()
       │         └─ vLLM/SGLang: 释放 KV cache GPU 内存
       │         └─ ModelEngineReplica → server.drain_and_lock.remote()
       │              └─ ModelEngineServer.drain_and_lock()
       ...
       │            └─ ModelEngineWorker 被分配独立的 NCCL rank，加入统一通信组
       │
       ├─ ④ trainer.update_weights() + rollout.update_weights()
       │       └─ ModelEngineWorker.update_weights() [ONE_TO_ALL, blocking=False]
       ...
       │         └─ ModelEngineReplica → server.load_weights_and_unlock.remote()
       │              └─ ModelEngineServer.load_weights_and_unlock()
       │                   ├─ _resume_event.set()          ← 释放 _infer_lock
       │                   ├─ worker_group.load_staged_weights()
       │                   │    └─ ModelEngineWorker.load_staged_weights() [ONE_TO_ALL]
       │                   │         └─ ModelEngineServerAdapter.load_staged_weights()
...
**关键设计**：
- **统一 worker group**：`CheckpointEngineManager` 把所有 replica（vLLM/SGLang/OldLogProb）的 `workers` 合并为一个临时 `RayWorkerGroup`，以 `CheckpointEngineWorker` 为接口类型统一调度。`ModelEngineWorker` 继承 `CheckpointEngineWorker`，接口完全兼容，无需特殊处理
...
- **partial rollout 窗口期**：`abort_all_requests` 对 model_engine_server 是 no-op，使 vLLM/SGLang 被打断的样本在 ①→② 之间仍可向 ModelEngineServer 发请求，用当前权重完成 log prob 计算
...
- `ModelEngineReplica` 在 `rollout_replicas` 中，`CheckpointEngineManager` 统一管理其生命周期
                 ...
                 └─ for replica in rollout_replicas:
                      if hasattr(replica, "shutdown"):
                          await replica.shutdown()
                            └─ ModelEngineReplica.shutdown()
                                 └─ server.shutdown.remote()
                                      └─ ModelEngineServer.shutdown()
                                           ├─ _shutdown = True
...
**关键设计**：
- `ModelEngineServer` 生命周期完全归属 Rollouter 侧，由 `FullyAsyncRollouter.fit()` 的 `finally` 块触发关闭
- Trainer 侧完全不感知 model_engine_server 的存在

---

## 五、ModelEngineServer 内部架构

### 状态机

`ModelEngineServer` 围绕三个 asyncio 原语维护状态：
...
```
ModelEngineServer（Ray Actor，单线程 asyncio 事件循环）
│
...
│
└─ compute_log_prob() / pause_serving() / resume_serving()
...
输入: TensorDict (batch_size 个请求，每个 batch_size[0]=1)
  ...
  │
  ├─ worker_group.compute_log_prob(data)  ← 分发到 ModelEngineWorker（同步）
  │    └─ ModelEngineWorker.compute_log_prob(data)  [同步，blocking=True]
  │         └─ ModelEngineServerAdapter.compute_log_prob(data)  [同步]
  │              └─ training_worker.infer_batch(data)
  │
...
**注**：`compute_log_prob` 为同步方法。`ModelEngineServer.infer_batch` 通过 `run_in_executor` 在线程池中执行，不阻塞 asyncio 事件循环。`_infer_lock` 在 `_execute_batch` 中持有，确保推理与权重传输不并发，无需额外的 `_gpu_lock`。
...
| 流程 | 关键机制 |
|------|---------|
| **初始化** | `ModelEngineReplica.init_standalone()` 自建 resource pool，与 vLLM/SGLang 完全对称；`init_model()` 延迟到 actor 启动后执行 |
...
| Replica 类 | `vLLMReplica` / `SGLangReplica` | `ModelEngineReplica` |
