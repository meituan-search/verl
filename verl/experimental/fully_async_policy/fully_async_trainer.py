...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    def __init__(
        ...
        self._dynamic_scaling_policy_name: str = config.async_training.get(
            "dynamic_scaling_policy", "default"
        )
        ...
        self._standalone_rollout_only_hybrid: bool = (
            self.dynamic_scaling_enabled and config.rollout.nnodes == 0
        )
    ...
    async def _setup_hybrid_checkpoint_manager(self):
        ...
        needs_hybrid = (
            ...
            or self.config.async_training.get("use_dynamic_resource_scaling", False)
        )
        if not needs_hybrid:
            return
    ...
    async def fit_step(self, batch_dict: dict = None):
        ...
        with marked_timer("step", self.timing_raw):
            ...
            if self.dynamic_scaling_enabled and ctrl.policy.should_deactivate(
                ...
                self.dynamic_scale_ctx.last_deactivate_duration_s = time.time() - _deact_start
                print(
                    f"[FullyAsyncTrainer] step={self.current_param_version} "
                    f"deactivation took {self.dynamic_scale_ctx.last_deactivate_duration_s:.2f}s",
                    flush=True,
                )
    ...
    async def _fit_update_weights(self):
        ...
        # Debug: dump manager state before sync
        cm = self.checkpoint_manager
        hcm = self.hybrid_checkpoint_manager
        def _fmt_replicas(replicas):
            return [f"{type(r).__name__}(rank={getattr(r, 'replica_rank', '?')})" for r in replicas]

        print(
            f"[_fit_update_weights] step={self.current_param_version} "
            f"checkpoint_manager: backend={getattr(cm, 'backend', None)}, "
            f"replicas({len(cm.replicas if cm else [])})={_fmt_replicas(cm.replicas if cm else [])}"
        )
        print(
            f"[_fit_update_weights] step={self.current_param_version} "
        ...
        _expect_samples = self.required_samples * self.trigger_parameter_sync_step * _completed_steps
        _buffer_sampels = self.required_samples * self.trigger_parameter_sync_step * self.staleness_threshold
        ...
        with marked_timer("timing_s/param_sync", self.timing_raw):
            ...
            print("zs-log start self.checkpoint_manager.update_weights!")
            ...
            if self.dynamic_scaling_enabled and should_activate:
                ...
                print("zs-log start self.hybrid_checkpoint_manager.update_weights!")
        ...
        print("zs-log end _fit_update_weights!")
