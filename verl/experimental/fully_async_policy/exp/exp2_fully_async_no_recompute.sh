#!/usr/bin/env bash
# Experiment 2: Fully Async + No Recompute (bypass_mode=True)
#
# Purpose: Establish fully_async throughput baseline without any recomputation.
#          old_log_probs are taken directly from rollout-time vLLM log probs (bypass mode).
#          GPU total matches Exp1 (32 GPUs) for fair comparison.
#
# GPU layout:
...
#
# Key settings:
#   - algorithm.rollout_correction.bypass_mode=True  (no recompute)
#   - model_engine_server.enable_standalone=False     (no dedicated server)
#   - actor_rollout_ref.rollout.calculate_log_probs=True  (vLLM computes log_probs during gen)
#
# Metrics to record (vs Exp1):
#   - End-to-end throughput speedup
#   - Per-step time breakdown (rollout / training / idle / param sync)
#   - GPU utilization (rollout GPU vs training GPU)
#   - Staleness ratio: fully_async/count/staleness_samples
...
exp_name='exp2_fully-async_no-recompute_dapo_qwen2.5-7B-math_megatron'
