...
> **Registration note:** `fixed_ratio` is **not** imported in `__init__.py`, so `@register_policy` does not fire on a default import. Add `from .fixed_ratio_policy import FixedRatioDynamicScalingPolicy` to `__init__.py`, or import it manually in your entry script, before referencing it in config.
...
`fully_async/*` metrics documented in [`docs/advance/fully_async.md`](../../../../docs/advance/fully_async.md))
...
- Numerator (compute time): $C_i = \sum_{k \in K} \text{timing\_raw}_i[k]$, where
  ...
  (missing keys — e.g. `values`/`update_critic` when `use_critic=False` — default to 0)
...
The Rollouter records an event-driven history of `len(active_tasks)` — appended every time a
sample is submitted, completes, or is drained during a pause — giving timestamps
$t_0 < t_1 < \dots < t_m$ with active-task counts $A_k$ holding over $[t_k, t_{k+1})$ ($t_0$ is the
previous step's end / this step's start; $t_m$ is "now", appended when `reset_staleness()` is
called, closing out the window including any trailing drain/idle time).

The max concurrency capacity is approximated using the value at the **end** of the window
(a deliberate approximation — it is not recomputed per sub-interval):

$$
S = \text{get\_active\_server\_count}() \times \text{concurrent\_samples\_per\_replica}
$$

For each interval $k$, the instantaneous utilization is $u_k = \min(1, A_k / S)$ (clamped because
concurrency can transiently exceed capacity under dynamic resource scaling). Weighting each
...
Returns `0.0` if there is no time span to integrate over (e.g. $S = 0$).
...
- **`rollout_resource_utilization` capacity approximation**: since $S$ is taken at the end of the
  window, a cycle in which Hybrid replicas activate/deactivate multiple times (causing $S$ to
  change significantly mid-window) can skew earlier intervals' utilization.
