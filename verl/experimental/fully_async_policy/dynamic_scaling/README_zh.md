...
本 cycle 从未把 Hybrid GPU 切入 rollout（如功能关闭、或跳过 `wait_for_enough_samples`）时 $x = 0$（Hybrid 时间 100% 用于训练）。

按各自测量的 GPU-秒 加权——Hybrid GPU 在 $(1-x) \cdot a$ GPU 时间内以 `train_resource_utilization` 训练、在 $x \cdot a$ GPU 时间内以 `rollout_resource_utilization` 做 rollout；Standalone GPU（$b$ 个）全部时间以 `rollout_resource_utilization` 做 rollout：
...
Trainer `fit_step()`（含 partial-rollout 恢复）结束后，MessageQueue 中剩余样本数的**快照**（非平均）：
