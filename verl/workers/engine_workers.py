...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def train_batch(self, data: TensorDict) -> TensorDict:
        ...
        import os as _os
        if _os.environ.get('DEBUG_DATA_ORDER') == '1':
            import torch as _torch, torch.distributed as _dist, hashlib as _hl
            if not _dist.is_initialized() or _dist.get_rank() == 0:
                try:
                    _ids = data['input_ids']
                    # Per-sample fingerprint to identify which samples are in the batch
                    _per = []
                    try:
                        # NestedTensor jagged layout: use offsets to split .values()
                        _vals = _ids.values()
                        _offs = _ids.offsets()
                        for _i in range(len(_offs) - 1):
                            _sv = _vals[_offs[_i]:_offs[_i+1]]
                            _per.append(_hl.md5(_sv.cpu().numpy().tobytes()).hexdigest()[:6])
                    except Exception:
                        for _s in _ids:
                            _per.append(_hl.md5(_s.reshape(-1).cpu().numpy().tobytes()).hexdigest()[:6])
                    # Also log prefix_segments turn counts per sample
                    _segs = tu.get_non_tensor_data(data, 'prefix_segments', None)
                    def _unwrap(x):
                        from tensordict.tensorclass import NonTensorData
                        return x.data if isinstance(x, NonTensorData) else x
                    _seg_info = [len(_unwrap(_segs[_i])) if _segs is not None else '?' for _i in range(len(_per))]
                    print(f'[DATA_ORDER] n={len(_per)} hashes={_per} n_segs={_seg_info}', flush=True)
                except Exception as _e:
                    print(f'[DATA_ORDER] fingerprint error: {_e}', flush=True)
        ...
        # Pre-compute tree FLOP ratio from actual batch data when prefix-tree is active.
        # Stored in data so postprocess_micro_batch can inject it into metrics.
        if self.engine_config.use_prefix_tree:
            try:
                from verl.utils.prefix_tree_flops import compute_tree_flop_ratio_from_batch
                _flop_ratio = compute_tree_flop_ratio_from_batch(data)
                if _flop_ratio is not None:
                    tu.assign_non_tensor(data, tree_flop_ratio=_flop_ratio)
            except Exception:
                pass
        ...
        # COMPARE_LAYERS: save fwd+bwd stats AFTER backward completes
        import os as _os
        _cl = _os.environ.get('COMPARE_LAYERS', '0')
        if _cl in ('1', '2'):
            import verl.models.mcore.prefix_tree_merge as _mp
            import json as _json, torch as _torch, hashlib as _hl
            if _mp._layer_capture:
                _use_pt = getattr(self.engine_config, 'use_prefix_tree', False)
                _tag = 'magi' if _use_pt else 'fa3'
                _logdir = _os.environ.get('COMPARE_LAYERS_DIR',
                    '/home/hadoop-djst-algoplat/verl-prefix-tree/.jobs/job_5/task_1/log')
                _os.makedirs(f'{_logdir}/{_tag}', exist_ok=True)
                # Print data fingerprint so we can verify same batch
                try:
                    _ids = data['input_ids']
                    _flat = _ids.values() if hasattr(_ids, 'values') else _ids.reshape(-1)
                    _h = _hl.md5(_flat.cpu().numpy().tobytes()).hexdigest()[:8]
                    print(f"[COMPARE_LAYERS] tag={_tag} input_ids_hash={_h} n_layers={len(_mp._layer_capture)}", flush=True)
                except Exception:
                    pass
                # Save summary JSON — keys are like "fa3_0", "magi_3"; skip "_bwd" suffixed keys
                _fwd_keys = [k for k in _mp._layer_capture if not k.endswith('_bwd')]
                _n = max(int(k.rsplit('_', 1)[1]) for k in _fwd_keys) + 1
                _data = {str(i): _mp._layer_capture.get(f'{_tag}_{i}') for i in range(_n)}
                _save = f'{_logdir}/{_tag}/layer_compare_{_tag}.json'
                with open(_save, 'w') as _f: _json.dump(_data, _f, indent=2)
                print(f"[COMPARE_LAYERS] Saved {_n} layer stats to {_save}", flush=True)
                # COMPARE_LAYERS=2: save full tensors
                if _cl == '2' and _mp._layer_tensors:
                    _tpath = f'{_logdir}/{_tag}/layer_tensors_{_tag}.pt'
                    _torch.save(_mp._layer_tensors, _tpath)
                    print(f"[COMPARE_LAYERS] Saved {len(_mp._layer_tensors)} tensors to {_tpath}", flush=True)
                    _mp._layer_tensors.clear()
        ...
        if self.engine.is_mp_src_rank_with_outputs():
            ...
            # Inject tree_flop_ratio (pre-computed from batch data, replaces prefix_sharing)
            _tree_ratio = tu.get_non_tensor_data(data, 'tree_flop_ratio', None)
            if _tree_ratio is not None:
                output["metrics"]["tree_flop_ratio"] = _tree_ratio
    ...
    def infer_batch(self, data: TensorDict) -> TensorDict:
        ...
        default_keys = dict(
            ...
            use_fused_kernels=self.engine_config.use_fused_kernels,
            # Val always uses standard FA3 attention — measures real inference quality
