...
class RolloutConfig(BaseConfig):
    ...
    # Mirror flag: emit per-token weight-version bookkeeping (token_versions)
    # from the rollout client. Only trainers that consume token-level
    # staleness diagnostics (partial_reprefill) need it; every other trainer
    # (colocate_async, reprefill_decoupled, ...) skips the per-trajectory
    # list construction entirely.
    emit_token_versions: bool = False
