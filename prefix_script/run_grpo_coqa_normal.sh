#!/usr/bin/env bash
# GRPO CoQA — baseline, Qwen3-4B-Base, Megatron TP=4, NO prefix tree, 100 steps
# Same backend as MAGI for fair comparison
...
OUTDIR="${OUTDIR:-/tmp/claude/grpo_coqa_4b/baseline/${TS}}"
mkdir -p "$OUTDIR"
...
echo "================================================================"
echo "GRPO CoQA — BASELINE (Megatron, no prefix tree)"
echo "  model: $MODEL_PATH  steps: 100"
echo "================================================================"
...
TENSORBOARD_DIR="$OUTDIR/tb" python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=coqa_base_4b \
    trainer.save_freq=-1 \
    trainer.test_freq=-1 \
