#!/usr/bin/env bash
# GRPO LongReason — fixed mbs=8 + prefix-tree MAGI attention, Qwen3-1.7B-Base
# TP=1, CP=8 — fixed micro-batch size (no dynbsz) to test ppo_kl grouping hypothesis
...
OUTDIR="${OUTDIR:-/tmp/claude/grpo_longreason/magi_mbs8_${SPLIT}_${TS}}"
mkdir -p "$OUTDIR"

echo "================================================================"
echo "GRPO LongReason — MAGI fixed-mbs8  SPLIT=$SPLIT  TS=$TS"
echo "  model: $MODEL_PATH"
echo "  prompt_len=$MAX_PROMPT  response_len=$MAX_RESPONSE"
echo "  micro_batch_size=8 (fixed, no dynbsz)  vllm_max_len=$MAX_MODEL_LEN"
echo "  output: $OUTDIR"
echo "================================================================"
...
TENSORBOARD_DIR="$OUTDIR/tb" python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="longreason_${SPLIT}_magi_mbs8_${TS}" \
    trainer.save_freq=-1 \
    trainer.test_freq=-1 \
