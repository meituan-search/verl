#!/usr/bin/env bash
# GRPO LongReason — dynbsz baseline (no prefix tree), Qwen3-1.7B
...
unset NVTE_FLASH_ATTN NVTE_FUSED_ATTN NVTE_UNFUSED_ATTN 2>/dev/null || true
...
OUTDIR="${OUTDIR:-/tmp/claude/grpo_longreason/baseline_${SPLIT}_${TS}}"
mkdir -p "$OUTDIR"


echo "================================================================"
echo "GRPO LongReason — BASELINE dynbsz  SPLIT=$SPLIT  TS=$TS"
...
TENSORBOARD_DIR="$OUTDIR/tb" python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.use_prefix_tree=False \
    \
    ...
    trainer.project_name="grpo_longreason_1.7b_baseline" \
    trainer.experiment_name="longreason_${SPLIT}_baseline_${TS}" \
    trainer.save_freq=-1 \
    trainer.test_freq=-1 \
