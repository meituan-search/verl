#!/usr/bin/env bash
# GRPO | Qwen3-8B | Megatron training | staleness_sweep trainer
#
# Experimental V1 PPO trainer mode that diagnoses off-policy effects by
# sweeping staleness 0..N-1 across N train steps within a cycle. At cycle
# start, one-shot rollout of N*train_batch at W_0; subsequent N-1 steps
# consume one train_batch each from the pre-generated pool. At step k,
# reprefill at W_{k-1} produces new_rollout_log_probs; compute_offpolicy_metrics
# decomposes into staleness/* + mismatch/* + combined/* groups.
#
# Enable via trainer.v1.trainer_mode=staleness_sweep + trainer.v1.staleness_sweep.num_steps=N.
#
...
# staleness_sweep cycle length. At step k (k=1..N) within a cycle, samples
# are (k-1)-step stale. Sweeps staleness 0..N-1 every N steps.
...
experiment_name=${EXPERIMENT_NAME:-qwen3_8b_${INFER_BACKEND}_megatron_${DATASET}_staleness_sweep_n${NUM_STEPS}}
...
TRAINER=(
    ...
    trainer.v1.staleness_sweep.num_steps=${NUM_STEPS}
)
