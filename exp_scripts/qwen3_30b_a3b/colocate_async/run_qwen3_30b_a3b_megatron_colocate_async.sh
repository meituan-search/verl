#!/usr/bin/env bash
# GRPO | Qwen3-30B-A3B (MoE) | Megatron training | NVIDIA GPUs | colocate_async
...
#   NUM_WARMUP_BATCHES     pre-fill pipeline for rollout/training overlap (default: 1)
#
...
train_files=${TRAIN_FILES:-${DAPO_MATH_TRAIN:-"$DAPO_DIR/dapo-math-17k.parquet"}}
val_files=${VAL_FILES:-${AIME_VAL:-"$DAPO_DIR/aime-2024.parquet"}}
...
experiment_name=${EXPERIMENT_NAME:-qwen3_30b_a3b_${INFER_BACKEND}_megatron${ROLLOUT_QUANTIZATION:+_${ROLLOUT_QUANTIZATION}}_colocate_async}
########################### end user-adjustable ###########################
