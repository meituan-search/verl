...
Verifies:
- _build_global_trie attaches trie (TrieNode root) and leaf_idx (torch.long tensor)
- leaf_idx survives torch indexing through reordering (the contract TensorDict.reorder uses)
...
# ── Minimal TensorDict stand-in (tests the contract) ────────────────────────
class _BatchStandIn:
    """Minimal stand-in for DataProto's ``batch`` tensor propagation.

    Mirrors TensorDict reorder/chunk semantics: tensors in ``batch`` are
    indexed via ``val[indices]`` (torch indexing), so a ``torch.long``
    ``leaf_idx`` tensor propagates correctly through reorder and chunk.
    """
    ...
    def reorder(self, indices: torch.Tensor):
        ...
        for key, val in self.batch.items():
            if isinstance(val, list):
                reordered[key] = [val[i] for i in indices.tolist()]
            else:
                reordered[key] = val[indices]
        self.batch = reordered

    def chunk(self, chunks: int):
        n = len(next(iter(self.batch.values())))
        per = n // chunks
        result = []
        for i in range(chunks):
            sub = {k: v[i * per : (i + 1) * per] for k, v in self.batch.items()}
...
def test_build_global_trie_attaches_trie_and_leaf_idx():
    """Building the trie should produce a non-empty trie and a valid leaf_idx tensor."""
    ...
    assert isinstance(leaf_idx, torch.Tensor)
    assert leaf_idx.dtype == torch.long
...
def test_leaf_idx_survives_reorder():
    """leaf_idx must propagate through torch-index reordering (TensorDict.reorder contract)."""
    seqs, seg_hashes, seg_lengths, _, _ = _make_seqs()
    trie, leaf_idx = _build_global_trie_local(seqs, seg_hashes, seg_lengths)

    batch = _BatchStandIn({"leaf_idx": leaf_idx.clone()})
    perm = torch.randperm(len(seqs))
    batch.reorder(perm)

    torch.testing.assert_close(batch.batch["leaf_idx"], leaf_idx[perm])


def test_leaf_idx_survives_chunk():
    """leaf_idx must propagate through chunking (TensorDict.chunk contract)."""
    seqs, seg_hashes, seg_lengths, _, _ = _make_seqs()
    trie, leaf_idx = _build_global_trie_local(seqs, seg_hashes, seg_lengths)
    ...
    assert len(chunks) == 2
    concatenated = torch.cat([c.batch["leaf_idx"] for c in chunks], dim=0)
    torch.testing.assert_close(concatenated, leaf_idx)
...
def test_leaf_idx_points_to_valid_leaf_nodes():
    ...
    for i, flat_idx in enumerate(leaf_idx.tolist()):
